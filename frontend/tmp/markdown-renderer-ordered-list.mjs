  ...
  lines.forEach((line, idx) => {
    ...
    if (ulMatch) {
      ...
      currentListItems.push(
        /* @__PURE__ */ import_react.default.createElement("li", { key: idx, className: "leading-relaxed" }, parseInline(content))
      );
      return;
    }
...
var topLevelOrderedItems = Array.from(
  html.matchAll(/<ol\b[^>]*>([\s\S]*?)<\/ol>/g),
  (match) => match[1]?.match(/<li\b/g)?.length ?? 0
)[0] ?? 0;
assert.equal(
  orderedListCount,
  1,
...
assert.equal(
  topLevelOrderedItems,
  3,
  `Expected three ordered list items, received ${topLevelOrderedItems}.
${html}`
);
