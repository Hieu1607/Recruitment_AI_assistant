...
- Fields:
  ...
  - storage_provider (enum: minio, required)
  - minio_bucket (string, required)
  - minio_object_key (string, required)
  - storage_uri (string, required)
  - mime_type (string, required, must be PDF)
  - upload_status (enum: uploaded, processing, processed, failed)
  - parse_status (enum: not_started, text_extracted, normalized, failed)
...
- Relationships:
  - 1-to-1 or 1-to-many with CandidateProfile revisions.
  - 1-to-many with ExtractionTrace.
- Validation Rules:
  - Accept only PDF content type.
  - storage_provider must be minio.
  - minio_bucket and minio_object_key must be present for every stored resume.
...
## 3. ExtractionTrace
- Purpose: Audit trail linking extracted fields to resume source blocks.
- Fields:
  - id (UUID)
  - resume_document_id (UUID, required)
  - candidate_profile_id (UUID, nullable until profile exists)
  - source_page (integer, required)
  - source_bbox (json, required)
  - source_text_snippet (text, required)
  - mapped_field_name (string, required)
  - confidence_score (numeric, nullable)
  - created_at (timestamp)
- Validation Rules:
  - source_text_snippet must be masked in operational logs.
